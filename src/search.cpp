...
int16_t Engine::negamax(Board &board, int depth, int alpha, int beta, int16_t ply, bool nmpAllowed, bool isCutNode) {
    ...
    if(!inSingularSearch && staticEval - (rfpMultiplier.value * (depth - improving) + 20 * opponent_worsening) >= beta && !inCheck && depth < rfpDepthCondition.value && !isPV) return (staticEval + beta) / 2;
