...
void Engine::scoreMoves(const Board& board, std::array<Move, 256> &moves, std::array<int, 256> &values, int numMoves, Move ttMove, int16_t ply) {
    ...
    for(int i = 0; i < numMoves; i++) {
        ...
        if(move == ttMove) {
            ...
            // Capthist!
            values[i] = MVV_values[victim]->value + noisyHistoryTable[colorToMove][piece][board.squareIsUnderAttack(start)][end][victim][board.squareIsUnderAttack(end)];
            // see!
...
void Engine::updateNoisyHistory(const int colorToMove, const int piece, const int end, const int victim, const int bonus, const bool startAttack, const bool endAttack) {
    const int thingToAdd = bonus - noisyHistoryTable[colorToMove][piece][startAttack][end][victim][endAttack] * std::abs(bonus) / historyCap;
    noisyHistoryTable[colorToMove][piece][startAttack][end][victim][endAttack] += thingToAdd;
}
...
int16_t Engine::negamax(Board &board, int depth, int alpha, int beta, int16_t ply, bool nmpAllowed, bool isCutNode) {
    ...
    for(int i = 0; i < totalMoves; i++) {
        ...
        bool moveStartAttack = board.squareIsUnderAttack(moveStartSquare);
        ...
        if(legalMoves == 1) {
            ...
            if(depth > lmrDepth.value) {
                ...
                if(isQuiet) {
                    ...
                    lmr -= noisyHistoryTable[1 - board.getColorToMove()][getType(movedPiece)][moveStartAttack][moveEndSquare][moveVictim][moveEndAttack] / int(cmrDivisor.value);
                }
                lmr += isCutNode * 2;
                lmr -= improving;
            ...
            if(score > alpha) {
                ...
                if(score >= beta) {
                    ...
                    if(isQuiet) {
                        ...
                        updateHistory(colorToMove, start, end, piece, bonus, ply, hash, moveStartAttack, moveEndAttack);
                        ...
                        const int piece = getType(board.pieceAtIndex(move.getStartSquare()));
                        const int victim = getType(board.pieceAtIndex(end));
                        updateNoisyHistory(board.getColorToMove(), piece, end, victim, bonus, moveStartAttack, moveEndAttack);
                    }
                    bonus = -bonus;
                    // malus!
                    for(int moveNo = 0; moveNo < legalMoves - 1; moveNo++) {
                        ...
                        if(maluIsQuiet) {
                            ...
                            const int victim = getType(board.pieceAtIndex(end));
                            updateNoisyHistory(colorToMove, piece, end, victim, bonus, board.squareIsUnderAttack(start), board.squareIsUnderAttack(end));
                        }
                    }
                    break;
                }
