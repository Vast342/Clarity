...
int16_t Engine::negamax(Board &board, int depth, int alpha, int beta, int16_t ply, bool nmpAllowed, bool isCutNode) {
    ...
    while (true) {
        ...
        // move loop prunings:
        // futility pruning
        if(!picker.skipQuiets && bestScore > matedScore && !inCheck && depth <= fpDepthCondition.value && staticEval + fpBase.value + depth * fpMultiplier.value <= alpha) picker.skipQuiets = true;
        // Late Move Pruning
        ...
        // History Pruning
        if(ply > 0 && !isPV && isQuiet && depth <= hipDepthCondition.value && moveValue < hipDepthMultiplier.value * depth) break;
