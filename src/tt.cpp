...
void TranspositionTable::clearTable(const int threadCount) {
    ...
    for(int i = 0; i < threadCount; ++i) {
        threads.emplace_back([i, chunks, this]() {
            ...
            const std::size_t clearSize = std::min(chunks, size - clearIndex) * sizeof(Transposition);
...
void TranspositionTable::resize(const size_t newSizeMB, const int threadCount) {
    const size_t newSizeB = newSizeMB * 1024 * 1024;
    const size_t newSizeEntries = newSizeB / sizeof(Transposition);
    size = newSizeEntries;
